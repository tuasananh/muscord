...
export const r34FiltersUpdateModal: Modal = {
	data: (() => {
		...
		const keyInput = new TextInputBuilder()
			...
			.setLabel("Filter Key")
			.setPlaceholder("(New key will be added to filter list)");
	...
	run: async (c, interaction) => {
		const filters = ((await c.env.KV_STORE.get("filters", "json")) ||
			// eslint-disable-next-line @typescript-eslint/no-explicit-any
			{}) as any;

		if (
			interaction.data.components[0].type != ComponentType.ActionRow ||
		...
		const key = interaction.data.components[0].components[0].value;
		let value = interaction.data.components[1].components[0].value;
		const toAppend = interaction.data.components[2].components[0].value;
		...
		if (
			...
			filters[key] = value;
			await c.env.KV_STORE.put("filters", JSON.stringify(filters));
		}

		const content = "Updated `" + key + "` to `" + value + "`.";
